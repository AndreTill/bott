import type { APIMethodParams, APIMethodReturn, APIMethods, TelegramInputMedia, TelegramParams, TelegramResponseParameters, TelegramUpdate } from "@gramio/types";
...
export interface BotLike {
    ...
    downloadFile(attachment: Attachments.Attachment | {
        file_id: string;
 ...
 *
 *  */
export type ContextType<Bot extends BotLike, Name extends keyof ContextsMapping<Bot>> = InstanceType<ContextsMapping<Bot>[Name]>;
...
export type UpdateName = Exclude<keyof TelegramUpdate, "update_id"> | MessageEventName | CustomEventName;
/** Type helper to join union type */
export type JoinUnion<T> = T extends infer U ? U extends any ? // biome-ignore lint/suspicious/noExplicitAny: <explanation>
Record<string, any> & U : never : never;
/** Type helper to add array and non-array type */
export type MaybeArray<T> = T | T[];
/** Union type of attachments type */
export type AttachmentType = TelegramInputMedia["type"] | "sticker" | "video_note" | "voice" | "contact" | "poll" | "venue" | "location" | "story";
...
export type Require<O, K extends keyof O> = {
...
export type Optional<T, K extends keyof T> = /** We pick every field but `K` and leave them as is */ Pick<T, Exclude<keyof T, K>> /** Then, we take our `K` fields and mark them as optional */ & {
    [P in K]?: T[P];
...
export type tSendAnimation = id<"animation", TelegramParams.SendAnimationParams>;
...
export type tSendVideoNote = id<"video_note", TelegramParams.SendVideoNoteParams>;
...
export type tSendMethods = tSendAnimation | tSendAudio | tSendDocument | tSendPhoto | tSendSticker | tSendVideo | tSendVideoNote | tSendVoice;
