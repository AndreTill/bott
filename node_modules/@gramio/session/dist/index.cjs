...
function createProxy(value, onUpdate, sessionKey) {
  ...
  return new Proxy(value, {
    get(target, key) {
      ...
      return gramio.isPlainObject(value2) || Array.isArray(value2) ? createProxy(value2, onUpdate) : value2;
    },
...
function session(options = {}) {
  ...
  return new gramio.Plugin("@gramio/session").derive(
    [
      "message",
      "callback_query",
      "channel_post",
    ...
    async (context) => {
      ...
      const sessionData = await storage$1.get(sessionKey) ?? // @ts-ignore
      (options.initial && await options.initial(context)) ?? {};
      const onUpdate = () => storage$1.set(sessionKey, session2);
